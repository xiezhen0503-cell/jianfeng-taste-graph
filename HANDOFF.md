...
请不要重新安装依赖或运行 Next.js 构建。仓库内的 `site/` 已经是构建完成的纯静态网站：

1. 将 `site/` 内全部文件上传到云端静态站点根目录。
...
当前完整源码位于 `source/`。请优先读取该目录，不要反编译 `site/_next/`，也不要使用仓库中的历史 zip/part 文件。
...
- [ ] `/admin/` 返回 200。
- [ ] 首页点击“它怎么懂我？”能打开算法说明。
- [ ] 首次访问优先出现测试邀请，15 道题能完成并生成口味称号。
- [ ] 完成测试后可以领取优惠券。
- [ ] Taste 页面可以重新测试、生成并下载 PNG Taste Card。
- [ ] 发现页 Swipe 后推荐卡切换并显示学习反馈。
- [ ] 底部导航可以进入饮食搭配和厨房挑战。
- [ ] 3 天与 7 天动态搭配能随 Taste Graph 变化。
- [ ] 厨房挑战可以完成食材、顺序、火候、调味和装盘五步。
